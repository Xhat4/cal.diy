...
RUN npx turbo@2.10.12 prune --scope=@calcom/web --scope=@calcom/trpc --docker
RUN yarn install
...
ARG NEXT_PUBLIC_STRIPE_PUBLIC_KEY
ARG STRIPE_WEBHOOK_SECRET
ARG PAYMENT_FEE_FIXED=0
ARG PAYMENT_FEE_PERCENTAGE=0
...
CMD ["/bin/sh", "-c", "yarn workspace @calcom/prisma db-deploy && /calcom/scripts/start.sh"]
